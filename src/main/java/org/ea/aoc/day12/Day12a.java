...
public class Day12a {
    public static void main(String[] args) throws Exception {
        List<String> lines = Util.readInput(true, 12, false);
    ...
    private static void calcPos(Plant[] characterMap, int x, int y, int width, int height) {
        ...
        if (x == 0) plant.incWalls();
        if (y == 0) plant.incWalls();
        if (x == width - 1) plant.incWalls();
        if (y == height - 1) plant.incWalls();
    }
}
